...
if cfg.keep_track_invariants
    ...
    I0_nondim = abs( CGD.I(1,:) );
    I1_nondim = abs( CGD.I(2,:) );
    I2_nondim = abs( CGD.I(3,:) );
    I3_nondim = abs( CGD.I(4,:) );

    I0_nondim = abs( (I0_nondim-I0_nondim(1) ) / I0_nondim(1));
    ...
    vec_cell = {I0_nondim, I1_nondim, I2_nondim, I3_nondim};
    names_cell = {'I_0 (modulus of relative change in t)','I_1 (modulus of relative change in t)','I_2 (modulus of relative change in t)','I_3 (modulus of relative change in t)'};
...
if cfg.keep_track_amplific_factor || cfg.do_amplific_factor
    [TAF,IAF] = AmplificationFactor(CP,SD,CGD,state_old);

    CGD.total_amplific_factor = [CGD.total_amplific_factor TAF];
    CGD.total_amplific_factor = max(CGD.total_amplific_factor);

    CGD.inhomogeneity_amplific_factor = [CGD.inhomogeneity_amplific_factor IAF];
    CGD.inhomogeneity_amplific_factor = max(CGD.inhomogeneity_amplific_factor);
end
...
if cfg.compare2exact % L2 and Linfty errors for U and Phi
    ...
    Col_2 = [CGD.Linf_err_U(end); CGD.Linf_err_Phi(end)];
     ...
     if cfg.plot_final_error
         xy_saveplot_fig(SD.x,SD.y,real(state_old.U - CP.ExactSolution(SD.X,SD.Y,state_old.t)),'Error of u(x,y,T)','4_Final_Error_u',cfg,'x','y');
     end
