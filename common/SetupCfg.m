...
% create new job tag
job_tag = datestr(now, 'dd-mmm-yyyy_HH-MM-SS-FFF');
cfg.job_tag = job_tag;
...
cfg.outpath = out_folder_name;
diary(fullfile(cfg.outpath, 'everything.log'))

%% timestap and system diagnostics, moved here to be stored in the correct log


% timestamp of starting up the job
disp(['[SetupCfg] ' char(datetime('now')) ]);
...
try
    ...
    if ~isempty(hostname)
        disp(['[SetupCfg] Working on ' getHostname ]);
    end
catch ME
    disp('[SetupCfg] Couldn''t get hostname...');
end


% if desired, number of cores can be limited here
% maxNumCompThreads(8);


% print out cores info
disp('[SetupCfg] Processors info: ')
feature('numCores');
...
disp(['[SetupCfg] BLAS version: '  blas_info ]) 
...
cfg.init_type = 'advanced'; % options are 'naive', 'advanced', 'exact'. Controls how Phi^{-1/2} is computed
...
cfg.plot_final_posden = 1; % always one by deafult
